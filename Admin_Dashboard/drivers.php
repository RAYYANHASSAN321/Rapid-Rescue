<?php
    ...
    if(isset($_POST['create_driver'])){
      ...
      move_uploaded_file($imgpath, '../driver_image/'.$imagename);

      // Insert into the drivers table
      $query = "INSERT INTO drivers (D_name, phonenumber, d_email, d_password, d_image , ambulanceid) 
                VALUES ('$driverName', '$phoneNumber', '$email', '$password', '$imagename' , '$ambulance')";
...
<html lang="en">
  ...
  <div style="font-family: Cambria, Cochin, Georgia, Times, 'Times New Roman', serif;" class="container-fluid">
    <div class="row">
        ...
        <form style="padding: 30px; background: #b71c1c;" class="rounded-4" action="" method="POST" enctype="multipart/form-data">
    <div class="row text-start">
        <div class="col-6">
            <div class="input-field">
                <input type="text" name="driverName" required spellcheck="false">
                <label class="text-white">Driver Name</label>
            </div>
        </div>
        <div class="col-6">
            <div class="input-field">
                <input type="text" name="phoneNumber" required spellcheck="false">
    ...
    <div class="row text-start">
        <div class="col-6">
            <div class="input-field">
                <input type="email" name="driverEmail" required>
    ...
    <div class="row text-start">
        <div class="col-6">
            <div class="input-field">
                ...
                <input type="file" name="driverImage" required>
            </div>
        </div>
        ...
        <div class="col-6">
            ...
            <select name="ambulance">
                  <option hidden>Select Ambulance</option>
                  <?php
                  $sqlquery = "SELECT * FROM ambulances";
                  $run = mysqli_query($con , $sqlquery);
                  
                  if(mysqli_num_rows($run)){
                      while($row = mysqli_fetch_array($run)){
                  ?>
                  <option class="text-black fs-6"  value="<?php echo $row['ambulanceid']?>">
    ...
    <div class="row">
        <div class="col-12">
            <input style="background-color: #d18989;" class="btn btn-sm px-3 text-white fw-bold fs-5 w-100 border border-white border-1" name="create_driver" type="submit" value="Add Driver">
        </div>
    </div>
</form>
  ...
  <table class="table table-hover">
    <thead>
      <tr>
        <th style="background-color: #b71c1c;color: white;" class="border-bottom-5 border-danger">Driver Name</th>
        <th style="background-color: #b71c1c;color: white;" class="border-bottom-5 border-danger">Phone Number</th>
        <th style="background-color: #b71c1c;color: white;" class="border-bottom-5 border-danger">Email</th>
        <th style="background-color: #b71c1c;color: white;" class="border-bottom-5 border-danger">Image</th>
        <th style="background-color: #b71c1c;color: white;" class="border-bottom-5 border-danger">Action</th>
      </tr>
    </thead>
    <tbody>
      <?php
        $queryRead = "SELECT * FROM drivers";
        $queryRun = mysqli_query($con, $queryRead);
        if (mysqli_num_rows($queryRun)) {
          while ($row = mysqli_fetch_array($queryRun)) {
      ?>
      <tr>
        <td class="border-bottom-5"><?php echo $row['D_name']; ?></td>
        <td class="border-bottom-5"><?php echo $row['phonenumber']; ?></td>
        <td class="border-bottom-5"><?php echo $row['d_email']; ?></td>
        <td class="border-bottom-5"><img src="<?php echo "images/" . $row['d_image']; ?>" width="50" height="50" alt="Driver Image"></td>
        <td class="border-bottom-5">
          <a class="btn btn-primary btm-lg border border-white border-1 px-3 w-100" href="DriverEdit.php?id=<?php echo $row['driverid']; ?>">Edit</a>
          <a class="btn btn-danger btm-lg border border-white border-1 px-3 w-100" href="DriverDelete.php?id=<?php echo $row['driverid']; ?>">Delete</a>
        </td>
      </tr>
      <?php
          }
